...
<%
	...
	UserVO userseq = (UserVO)request.getAttribute("userseq"); 
%>
<!DOCTYPE html>
<html>
  <head>
...
#menu_wrap {position:absolute;top:0;left:0;bottom:0;width:250px;height:500px;margin:10px 0 30px 10px;padding:5px;overflow-y:auto;z-index: 1;font-size:12px;border-radius: 10px;}
...
<title>MAU</title>
  ...
  <body>
   <div class="p-3 mb-2 " style="float: none; margin:100 auto; color: #3384C6; background-color:  #FEFFED;" >
    ...
    <!-- 지도이름 -->
<div>
<button class="btn btn-primary"  style="border-radius:20px; font-size : 15px; border-style: none; color: black;background-color: #F3B922; width: 100%; height:60px; margin-top: 10px;"  type="button" onclick="location.href='/mau/guiding/guider/<%=userseq.getUserSeqId()%>'"><%=userseq.getUserIcon()%> <%=userseq.getUserNickName()%>님이 지도</button><br><br>
</div>
...
<div class="map_leftb">
	<div class="" onclick="geolocation()"> 
		<img id="mapbutton" src="${pageContext.request.contextPath}/image/map/mylocation.png" alt="현재위치" height="50px">
	</div><br>
<!-- 지도공유 -->
	<div class="share-box">
	   <div class="share-btn link" onclick="copy_url()">
			<img id="mapbutton" src="${pageContext.request.contextPath}/image/map/share.png" alt="지도공유" height="50px">
			<input type="hidden" name="url" id="SharePlaceUrl" value="http://localhost:8181/mau/guiding/guider/<%=userseq.getUserSeqId()%>">
	   </div>
	</div>
</div><!-- map_leftb end -->
</div>
...
function ClickListener(seq,map, marker, infowindow) {
    return function() {
    	...
    	window.open("${pageContext.request.contextPath}/detailInfo/"+seq, "PopupWin", "width=1000,height=1000");
        console.log(seq);
    };
}
